...
//                .hint(new IDialogHint.Impl().hint("测2试"))
    ...
    private static class Builder {
        private InfoNotifier mDNotifier;
        private IDialogHint mHint;

        public Builder dNotifier(InfoNotifier notifier) {
            mDNotifier = notifier;
            return this;
        }
    ...
    private static class Dialog implements ITarget<InfoMsg.Invoker> {
        final InfoNotifier dNotifier = new InfoNotifier();
        private final MsgManager<InfoMsg.Invoker, InfoMsg.Callback> mManager = new MsgManager<>();
        ...
        @Override
        public void onSetupMsgProcessors(List<MsgProcessor<? extends InfoMsg.Invoker>> msgProcessors) {
            msgProcessors.add(new MsgProcessor<>(InfoMsg.PushMsg.class, msg -> System.out.println("显示弹窗:" + getHintFromMsg(msg))));
            msgProcessors.add(new MsgProcessor<>(InfoMsg.PopMsg.class, msg -> {
                System.out.println("关闭弹窗:" + getHintFromMsg(msg));
                mManager.sendMsg(new InfoMsg.OnDismissDialog(null));
            }));
        }

        private String getHintFromMsg(InfoMsg.Invoker<IDialogHint> msg) {
